 ...
 * @constructor
 */
module.exports = function ConnectingDataLink() {
...
module.exports.prototype.onDataChanged = function(event) {
  ...
  for (i = 0; i < event.added.length; i++) {
    if (!!event.added[i].target && !!event.added[i].source) {
      ...
      var pathMesh = babylon.MeshBuilder.CreateDisc(
        "t",
        {
          radius: event.added[i].radius,
          tessellation: 20,
      ...
      var distance =
        ...
        event.added[i].sourceGap -
        event.added[i].destGap;
      ...
      for (itemCounter = 0; itemCounter < numberOfItems; itemCounter++) {
        ...
        pathMeshArray[itemCounter].position = relativePosition
          .scale(
            (itemCounter + 1) * event.added[i].distance +
              event.added[i].sourceGap
          )
          .addInPlace(source.position);
      }

      //Update when the item is dragged
      if (target.data.item.onDrag) {
        this.decorateTargetOnDrag(
          target,
          source,
          pathMesh,
          pathMeshArray,
          event.added[i].distance,
          event.added[i].sourceGap,
          event.added[i].destGap
        );
      }
...
module.exports.prototype.decorateTargetOnDrag = function(
  target,
  source,
  pathMesh,
  pathMeshArray,
  ...
  target.data.item.onDrag = function(
    screenX,
    screenY,
    planarX,
    planarY,
    ...
    //Reap
    for (
      itemCounter = pathMeshArray.length;
      itemCounter > numberOfItems;
      itemCounter--
    ) {
    ...
    for (itemCounter = 0; itemCounter < pathMeshArray.length; itemCounter++) {
      pathMeshArray[itemCounter].position = relativePosition
        .scale((itemCounter + 1) * separation + sourceGap)
        .addInPlace(source.position);
    }
    //Increase
